...
const UA_LOCATIONS = [
  ...
  "Тернопіль","Чортків","Харків","Лозова","Чугуїв","Херсон","Хмельницький",
  "Кам'янець-Подільський","Черкаси","Умань","Золотоноша","Сміла","Канів",
  "Чернівці","Чернігів","Ніжин","Прилуки","Новосілки","Нові Безрадичі",
  "Старі Безрадичі","Рожівка","Проліски","Мила","Сквира","Ржищів","Кожанка",
  "Маньківка","Тетіїв","Бориспільський район","Вишгородський район",
  "Броварський район","Бучанський район","Обухівський район","Фастівський район",
  "Васильківський район","Білоцерківський район","Переяславський район"
];
...
function setCurrency(cur) {
  ...
  document.querySelectorAll(".cur-tab").forEach(b => b.classList.toggle("active", b.textContent.includes(cur)));
  const hints = { USD:"1 $ ≈ 41 ₴ ≈ 0.93 €", EUR:"1 € ≈ 44 ₴ ≈ 1.07 $", UAH:"1 000 ₴ ≈ 24 $ ≈ 22 €" };
...
function filterLocationSuggestions() {
  ...
  box.innerHTML = "";
  if (!q) return;
  const matches = UA_LOCATIONS.filter(l => l.toLowerCase().includes(q) && !selectedLocations.includes(l)).slice(0,10);
  if (!matches.length) {
...
function renderSelectedLocations() {
  ...
  if (btn) {
    btn.classList.toggle("active", selectedLocations.length > 0);
    btn.innerHTML = selectedLocations.length > 0
      ? `📍 ${selectedLocations.length} район(и) <span class="arrow">▼</span>`
      : `📍 Район <span class="arrow">▼</span>`;
  }
}
...
function getAll_Land() {
  ...
  return all.filter(p => p.type && p.type.toLowerCase().includes("ділянка"));
}
...
function applyFilters() {
  ...
  // Призначення
  const purposeChecks = [...document.querySelectorAll(".purpose-check:checked")].map(c => c.value);

  filteredData = items.filter(h => {
    ...
    if (purposeChecks.length > 0) {
      const desc = ((h.description||"")+(h.title||"")).toLowerCase();
      const ok = purposeChecks.some(p => desc.includes(p));
      if (!ok) return false;
    }
...
function renderPage() {
  ...
  if (!page.length) {
    grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">🌳</div><h3>Ділянок не знайдено</h3><p>Спробуйте змінити параметри фільтрування</p></div>`;
    return;
  }
  ...
  page.forEach((h,i) => {
    ...
    card.innerHTML = `
      <div class="card-img-wrap">
        <img src="${h.image||'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&q=80'}" alt="${h.title}" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800&q=80'">
        <div class="card-badges"><span class="badge badge-type">Ділянка</span></div>
...
function renderActiveFilters() {
  ...
  const aMin = document.getElementById("areaMin").value;
  const aMax = document.getElementById("areaMax").value;
  if (aMin||aMax) { addTag(wrap,`📐 ${aMin||0}–${aMax||"∞"} сот.`,()=>{ document.getElementById("areaMin").value=""; document.getElementById("areaMax").value=""; applyFilters(); }); any=true; }
  [...document.querySelectorAll(".purpose-check:checked")].forEach(c => { addTag(wrap,`🌿 ${c.value}`,()=>{c.checked=false;applyFilters();}); any=true; });
...
function addTag(wrap,text,fn) {
  ...
  t.innerHTML=`${text} <button>×</button>`; t.querySelector("button").onclick=fn; wrap.appendChild(t);
}

function resetFilters() {
  ["priceMin","priceMax","areaMin","areaMax","searchHouses"].forEach(id=>{const el=document.getElementById(id);if(el)el.value="";});
  document.querySelectorAll(".purpose-check").forEach(c=>c.checked=false);
  selectedLocations=[]; renderSelectedLocations(); applyFilters();
}
